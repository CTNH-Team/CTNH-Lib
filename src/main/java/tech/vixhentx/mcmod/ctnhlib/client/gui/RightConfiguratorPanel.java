...
public class RightConfiguratorPanel extends WidgetGroup {
    ...
    public RightConfiguratorPanel(int x, int y) {
        super(x, y, 24, 0);
    }
    ...
    public class Tab extends WidgetGroup {
        ...
        private void expand() {
            ...
            // 向右展开的边界检查：防止超出屏幕右边界
            if (getParentPosition().x + getTabSize() + size.width > gui.getScreenWidth()) {
                this.dragOffsetX -= (getParentPosition().x + getTabSize() + size.width - gui.getScreenWidth());
            }

            // 底部边界检查
            if (getParentPosition().y + size.height > gui.getScreenHeight()) {
                this.dragOffsetY -= getParentPosition().y + size.height - gui.getScreenHeight();
            }

            // 向右展开：从标签右侧开始
            Position position = new Position(dragOffsetX + getTabSize(), dragOffsetY);
    ...
    public class FloatingTab extends Tab {
        ...
        public void collapseTo(int x, int y) {
            ...
            RightConfiguratorPanel.this.removeWidget(this);
            closeCallback.run();
        }
