...
public class LockIdentityHashMap<K, V> extends AbstractObject2ObjectSortedMap<K, V>
     ...
     */
    public LockIdentityHashMap() {
        this(Comparator.comparingInt(System::identityHashCode));
    }
     ...
     */
    public LockIdentityHashMap(final Comparator<? super K> c) {
        tree = null;
        count = 0;
        storedComparator = c;
        setActualComparator();
     ...
     */
    public LockIdentityHashMap(final Map<? extends K, ? extends V> m) {
        this();
        putAll(m);
    }
     ...
     */
    public LockIdentityHashMap(final SortedMap<K, V> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     */
    public LockIdentityHashMap(final Object2ObjectMap<? extends K, ? extends V> m) {
        this();
        putAll(m);
    }
     ...
     */
    public LockIdentityHashMap(final Object2ObjectSortedMap<K, V> m) {
        this(m.comparator());
        putAll(m);
    }
     ...
     * @throws IllegalArgumentException if {@code k} and {@code v} have different lengths.
     */
    public LockIdentityHashMap(final K[] k, final V v[]) {
        this(k, v, null);
    }
    ...
    private class TreeIterator {
        ...
        Entry<K, V> prev;
        /**
         * The entry that will be returned by the next call to {@link ListIterator#next()} (or
        ...
        Entry<K, V> curr;
        /**
         * The current index (in the sense of a {@link ListIterator}). Note that this value is not
        ...
        LockIdentityHashMap.Entry<K, V> previousEntry() {
        ...
        public void remove() {
            synchronized (LockIdentityHashMap.this) {
                ...
                updatePrevious();
                updateNext();
                LockIdentityHashMap.this.remove(curr.key);
                curr = null;
            }
        }
     ...
     *
     * <p>
     * In addition to the semantics of {@link Map#keySet()}, you can safely cast the set
     ...
     * the {@link ListIterator#next()}/{@link ListIterator#previous()} methods (and
     ...
     *
     * <p>
     * In addition to the semantics of {@link Map#values()}, you can safely cast the
     ...
     * {@link SortedMap#firstKey()} or {@link Collection#size()} must be always
     * computed on-the-fly.
     */
    private final class Submap extends AbstractObject2ObjectSortedMap<K, V> implements java.io.Serializable {
        ...
        boolean top;
        /** Cached set of entries. */
        protected transient ObjectSortedSet<Entry<K, V>> entries;
        /** Cached set of keys. */
        protected transient ObjectSortedSet<K> keys;
        /** Cached collection of values. */
        protected transient ObjectCollection<V> values;
        ...
        public Submap(final K from, final boolean bottom, final K to, final boolean top) {
            ...
            this.to = to;
            this.top = top;
            this.defRetValue = this.defRetValue;
        }
        ...
        final boolean in(final K k) {
            return (bottom || compare(k, from) >= 0) && (top || compare(k, to) < 0);
        }
        ...
        public ObjectSortedSet<Entry<K, V>> object2ObjectEntrySet() {
            if (entries == null) entries = new AbstractObjectSortedSet<Entry<K, V>>() {
                ...
                @Override
                public ObjectBidirectionalIterator<Entry<K, V>> iterator(final Entry<K, V> from) {
                    return new LockIdentityHashMap.Submap.SubmapEntryIterator(from.getKey());
                }

                @Override
                public Comparator<? super Entry<K, V>> comparator() {
                    return object2ObjectEntrySet().comparator();
                }
                ...
                @Override
                public boolean isEmpty() {
                    return !new LockIdentityHashMap.Submap.SubmapIterator().hasNext();
                }

                @Override
                public void clear() {
                    LockIdentityHashMap.Submap.this.clear();
                }
                ...
                @Override
                public ObjectSortedSet<Entry<K, V>> subSet(Entry<K, V> from, Entry<K, V> to) {
                    return subMap(from.getKey(), to.getKey()).object2ObjectEntrySet();
                }

                @Override
                public ObjectSortedSet<Entry<K, V>> headSet(Entry<K, V> to) {
                    return headMap(to.getKey()).object2ObjectEntrySet();
                }

                @Override
                public ObjectSortedSet<Entry<K, V>> tailSet(Entry<K, V> from) {
                    return tailMap(from.getKey()).object2ObjectEntrySet();
                }
            };
            return entries;
        ...
        private class KeySet extends AbstractObject2ObjectSortedMap<K, V>.KeySet {
            ...
            @Override
            public ObjectBidirectionalIterator<K> iterator(final K from) {
                return new LockIdentityHashMap.Submap.SubmapKeyIterator(from);
            }
        }
        ...
        public ObjectCollection<V> values() {
            if (values == null) values = new AbstractObjectCollection<V>() {
                ...
                @Override
                public int size() {
                    return LockIdentityHashMap.Submap.this.size();
                }
         ...
         */
        public LockIdentityHashMap.Entry<K, V> lastEntry() {
            if (tree == null) return null;
         ...
         * after a {@link ListIterator#next()} or {@link ListIterator#previous()}. If we
        ...
        private class SubmapIterator extends TreeIterator {
            ...
            void updatePrevious() {
                prev = prev.prev();
                if (!bottom && prev != null && compare(prev.key, from) < 0) prev = null;
            }

            @Override
            void updateNext() {
                next = next.next();
                if (!top && next != null && compare(next.key, to) >= 0) next = null;
            }
        }
         ...
         * override the {@link ListIterator#next()}/{@link ListIterator#previous()}
